...
class DoNotDeleteErrorHandler implements DatabaseErrorHandler {
	...
	public void onCorruption(SQLiteDatabase dbObj) {
		Log.e(TAG,
				"Corruption reported by sqlite on database: " + dbObj.getPath());
	}
}
...
public class CustomSqlite extends Activity {
	...
	public void thread_test_1() {
		...
		final SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DB_PATH,
				null);
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				SQLiteStatement st = db
						.compileStatement("SELECT sum(x+y) FROM t1");
	...
	public void run_my_test_sqlite(View view) {
		Intent intent = new Intent(getApplicationContext(), MyTestSQLite.class);
